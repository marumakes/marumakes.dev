...
export function gearTrain(teeth: number[], cy: number, startX: number): string {
  ...
  teeth.forEach((count, index) => {
    ...
    parts.push(
      ...
      stroked(
        "circle",
        { cx: x, cy, r: pitchRadius(count, TRAIN_MODULE) * 0.4 },
        "boss",
      ),
    ...
    if (index < teeth.length - 1) {
      x +=
        pitchRadius(count, TRAIN_MODULE) +
        pitchRadius(teeth[index + 1], TRAIN_MODULE);
    }
  });
...
export function makersMark(): string {
  ...
  const smallX =
    bigX +
    pitchRadius(bigTeeth, SEAL_MODULE) +
    pitchRadius(smallTeeth, SEAL_MODULE);
  const cy = 17;
  ...
  return [
    ...
    stroked(
      "polygon",
      { points: gearProfile(bigX, cy, bigTeeth, SEAL_MODULE, 0) },
      "tooth",
    ),
...
export function marginalWheel(teeth: number, spokes: number): string {
  ...
  const parts = [
    stroked(
      "polygon",
      { points: gearProfile(centre, centre, teeth, module, 0) },
      "ring",
    ),
...
function bandTop(index: number, count: number, random: () => number): number {
  ...
  const top =
    TOP_MIN + index * gap + wander(index, count, gap * JITTER_SPAN, random);

  return Number(top.toFixed(2));
}

export function marginalArrangement(
  seed: string,
  words: number,
): MarginalDrawing[] {
  if (words < MIN_WORDS) {
